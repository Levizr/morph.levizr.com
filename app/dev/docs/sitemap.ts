...
// Renders per request; registry data lives in the fetch data cache
// (DEV_NAV_TAG) and is purged by the Vercel purge workflow on every
// docs push — so the sitemap reflects the registry within
// milliseconds of a change, with no timers.
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return docs.map((doc) => {
    ...
    return {
      url: `${SITE_URL}/dev/docs/${doc.path}`,
